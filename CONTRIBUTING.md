...
```
$ git clone https://github.com/<your-username>/ics
$ cd ics
$ git remote add upstream https://github.com/RoadTripMoustache/ics
```
...
## Step 6 : Prepare your commit
In this step you will need to run several commands to be sure everything is ready before commiting.

- `dart format .` - To properly format dart files
- `flutter analyze` - To check if there's no lint issues
- `dart pub global run index_generator` - To regenerate the `lib/ics.dart` file

## Step 7 : Add and Commit changes
- IMPORTANT: You need to be in the project root folder _(not in `ics/example/` )_ for all git commands.
```
# To go back to the project root from example folder
$ cd ..
```
  > or you can just launch a new terminal window in `ics/` folder 
...
## Step 9 : Create the Pull Request (PR)
